...
<body>
	...
	<div class="produta">
		<div class="produta_til">
			<a href="open.html">卖家中心</a> < <span style="font-weight: 700;">退款问题在线帮助</span>
		</div>
		<jsp:include page="setupShopCommons/left.jsp" />
		<!--中间开始-->
		<div class="sold">
			<div class="top1">
				<form action="proscenium/searchRefund" method="get" id="ww">
					订单编号：<input type="text" name="orderFormID" /> 从：<input type="date"
						name="paymentDateLeft" />到：<input type="date"
						name="paymentDateRight" /><br /> 商品名称：<input type="text"
						name="nameOfGoods" /> 退款状态：<select name="orderStatusFrom">
						<option value="">全部</option>
						<option value="refundOrderForm">退款中</option>
						<option value="refundSuccess">退款关闭</option>
						<option value="refundFailed">退款成功</option>
					</select><br /> <br /> <input type="submit" value="搜索" />
				</form>
			</div>
			<div class="class2">
				...
				<div class="nav2" id="text1" style="display: block">
					<c:forEach items="${orderFormsOfUser}" var="o">
						<ul>
							...
							<li><c:if test="${o.orderstatus == 'refundOrderForm'}">退款中</c:if>
					...
					<c:forEach items="${orderFormsOfShop}" var="o">
						<c:if test="${o.orderstatus == 'ApplicationForRefund' }">
							<ul>
								...
								<li><c:if test="${o.orderstatus == 'ApplicationForRefund'}">申请退款</c:if>
								</li>
								<li><a onclick="a();">接收</a>&nbsp;&nbsp; <a onclick="b();">拒绝</a>
								</li>
								<script>
									function a() {
